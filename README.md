...
## Тестирование
**Команды**
- Дорохов С. - QA Team Lead and developer 
- Батуев М. -  QA Engineer

Настроен базовый процесс тестирования:

- Тестовая стратегия описана в файле `TESTING.md`.
- Типы автотестов:
    - developer (юнит тесты) в `tests/dev/`, запуск: `npm run test:dev`;
    - QA(функциональные тесты) в `tests/qa/`, запуск: `npm run test:qa`.
- Общий запуск тестов: `npm test`.
